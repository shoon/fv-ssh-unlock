			...
			if len(args) == 0 {
				...
				for _, device := range allDevices {
					if err := s.Remove(device.Name); err != nil {
						fmt.Printf("Error removing device '%s': %s\n", device.Name, terminalSafeInline(err.Error()))
					} else {
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, device := range devicesToUnlock {
				...
				if err != nil {
					...
					fmt.Printf("Enter password for %s@%s: ", device.User, device.Host)
					password, err = credentials.ReadPassword()
					if err != nil {
						return fmt.Errorf("error reading password: %w", err)
					}
				...
				for {
					...
					if verbose {
						fmt.Printf("[verbose] Attempt %d/%d: Unlocking %s@%s, successMsg=%q\n", attempts, maxAttempts, device.User, hostWithPort, successMsg)
					} else {
						fmt.Printf("Attempt %d/%d: Unlocking %s@%s\n", attempts, maxAttempts, device.User, hostWithPort)
					}
					...
					case fvcore.StatusUnlocked:
						...
						if verifyWindow > 0 {
							...
							case verr != nil:
								fmt.Printf("WARNING: could not verify %s: %s\n", device.Name, terminalSafeInline(verr.Error()))
							case ok:
					...
					default:
						if errors.Is(res.Error, fvcore.ErrHostKeyMismatch) {
							fmt.Printf("SECURITY ERROR: refusing %s: %s\n", device.Name, terminalSafeInline(res.Error.Error()))
							return res.Error
						}
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, d := range targets {
				...
				case errors.Is(perr, fvcore.ErrIndeterminate):
					...
					fmt.Printf("%-20s %s\n", d.Name, indeterminateStatusText)
				case perr != nil:
					fmt.Printf("%-20s unknown (%s)\n", d.Name, terminalSafeInline(perr.Error()))
				default:
					fmt.Printf("%-20s %s\n", d.Name, st)
				}
			}
