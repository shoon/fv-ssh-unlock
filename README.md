...
See [SECURITY.md](SECURITY.md) for private vulnerability reporting. The public
repository also uses GitHub secret scanning with push protection, Dependabot
security updates, and CodeQL analysis for Go and GitHub Actions. CI and release
workflows use read-only permissions by default and grant write access only to
the tagged release job.
